...
switch _mode do {
    case "init": {
        ...
        if (_isActivated) then {
            ...
            ] call FUNC(doVirtualArtillery);
            _state set ["module", _module];

            _module setVariable [QGVAR(ambientArtilleryVirtual_State), _state];
        } else {
            private _state = _module getVariable [QGVAR(ambientArtilleryVirtual_State), createHashMapObject];
            _state call ["fn_cancelArtillery"];
            _state = nil;
        };
    };
